...
class Vector_ElasticNet():
    ...
    'Regularization Paths for Generalized Linear Models via Coordinate Descent' - Jerome Friedman, Trevor Hastie*,
     and Rob Tibshirani'
    pero considerando un lambda distinto para cada feature
    '''

    def __init__(self, alpha, Lambda_, max_iter=1000, coef=None, intercept=None, eps=0.0001):
    ...
    def fit(self, X, y):
        ...
        #print('residuo inicial:', residuos)
        ...
        for it in range(self.max_iter):
            ...
            # residuos=[y[i]- (self.intercept+sum([X[i][j]*self.coef[j] for j in range(p)])) for i in range(n)]
            if not epsilon_equal(old_intercept, self.intercept, self.eps):
                cambio = True
                for i in range(n): residuos[i] += old_intercept - self.intercept
                #print('residuo despues de actualizar beta 0:', residuos)
            for j in range(p):

                z = sum(X[i][j] * residuos[i] for i in range(n)) / n + self.coef[j]
                #print('z:', z)
                old_coef = self.coef[j]
                self.coef[j] = self.soft_thresholding(z, self.Lambda_[j] * self.alpha) / (
                            1 + self.Lambda_[j] * (1 - self.alpha))
                if not epsilon_equal(old_coef, self.coef[j], self.eps):
                    cambio = True
